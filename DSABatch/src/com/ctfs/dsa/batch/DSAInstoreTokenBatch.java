package com.ctfs.dsa.batch;

import java.sql.SQLException;
import java.util.logging.Logger;

import com.ctal.utility.log.Log;
import com.ctfs.dsa.db.DSACustomerTransactionProcessor;
import com.ctfs.dsa.db.DSAInstoreTokenProcessor;
import com.ctfs.utility.exception.DBFailureException;


/**
 * 
 * Batch job invoked to delete data frm INSTOR_TOKEN
 * @authr
 *
 */
public class DSAInstoreTokenBatch {
	
	private static Logger log = Logger.getLogger(DSAInstoreTokenBatch.class.getName());
	
	public static void main(String[] args) {
		log.info("Entering insde the DSAInstoreTokenBatch main method");
		try {
			DSAInstoreTokenProcessor processor = new DSAInstoreTokenProcessor();
			processor.processData();
		} catch (SQLException | DBFailureException sqlexe) {
			Log.error("DB Failure while processing data", sqlexe);
			System.exit(1);
		} catch (Exception e) {
			Log.error("General exception occurred", e);
			System.exit(2);
		} 
		
		log.info("Exiting from DSAInstoreTokenBatch main method");
		System.exit(0);
	}
}
